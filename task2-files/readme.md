  ...
  deploy:
    ...
    steps:
      ...
      - name: deploy via helm
        run: |
          ssh -o StrictHostKeyChecking=no ${{ secrets.JUMP_SERVER_USER }}@${{ secrets.JUMP_SERVER_IP }} << EOF
          /usr/local/bin/helm upgrade --install simple-api /home/${{ secrets.JUMP_SERVER_USER }}/simple-api-chart \
            --reuse-values \
